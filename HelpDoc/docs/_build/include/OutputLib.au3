...
_OutputBuildWriteLine($sData) - Writes data to the build pane (by entire line).
_OutputBuildWriteLineError($sData) - Writes data to the build pane (by entire line if error).
...
;	True if successful, False otherwise.
; ===================================================================
Func _OutputBuildWriteLine($sData)
	$g_sOutputBuffer &= $sData
	Local $asLine = StringSplit($g_sOutputBuffer, @LF)
	If @error Then Return

	For $i = 1 To $asLine[0] - 1
		_OutputBuildWrite($asLine[$i] & @LF)
	Next
...
;	True if successful, False otherwise.
; ===================================================================
Func _OutputBuildWriteLineError($sData)
	$g_sOutputBuffer &= $sData
	Local $asLine = StringSplit($g_sOutputBuffer, @LF)
	If @error Then Return

	For $i = 1 To $asLine[0] - 1
		If StringLeft($asLine[$i], 4) <> "html" Then _OutputBuildWrite($asLine[$i] & @LF)
	Next
