...
Func _SetBitmapAlpha($hWnd, $hBitmap, $iAlpha)
	...
	Local $tRECT, $tSIZE, $W, $H, $hDC, $hDestDC, $hDestSv, $hSrcDC, $hSrcSv, $hObj, $hBmp
	...
	$hBmp = _WinAPI_CreateCompatibleBitmapEx($hDC, $W, $H, _WinAPI_SwitchColor(_WinAPI_GetSysColor($COLOR_3DFACE)))
	$hDestSv = _WinAPI_SelectObject($hDestDC, $hBmp)
	$hSrcDC = _WinAPI_CreateCompatibleDC($hDC)
	$hSrcSv = _WinAPI_SelectObject($hSrcDC, $hBitmap)
	_WinAPI_AlphaBlend($hDestDC, 0, 0, $W, $H, $hSrcDC, 0, 0, DllStructGetData($tSIZE, 'X'), DllStructGetData($tSIZE, 'Y'), $iAlpha, 0)
	_WinAPI_ReleaseDC($hWnd, $hDC)
	_WinAPI_SelectObject($hDestDC, $hDestSv)
	_WinAPI_DeleteDC($hDestDC)
	_WinAPI_SelectObject($hSrcDC, $hSrcSv)
	_WinAPI_DeleteDC($hSrcDC)
	$hObj = _SendMessage($hWnd, $STM_SETIMAGE, 0, 0)
	If $hObj Then
