...
#AutoIt3Wrapper_Res_Fileversion=1.6.6.3
...
Global $autoit3dir = RegRead("HKLM\Software\AutoIt v3\Autoit", 'InstallDir')
FileChangeDir($autoit3dir)
;
...
GUICtrlCreateLabel("AutoIt3Dir=" & $autoit3dir, $BaseX + 10, $BaseY + 18, 300, 20)
...
;
#region - Tools
If FileExists($autoit3dir & '\SciTE\Properties\au3.properties') Then
	Global $file_au3properties = $autoit3dir & '\SciTE\Properties\au3.properties'
	Global $comment = '#~ '
	$split = SciteToolsFileRead($file_au3properties)
	If @error Then
		ConsoleWrite('Error: FileRead ' & $file_au3properties & @LF)
		Exit
	EndIf
	...
	Next
Else
	; Use empty variables to disable operation of Tools Tab
	...
	If $rc Then
		...
		If $SciTE4AutoIt3Date = "" Then
			...
			If $SciTE4AutoIt3RegDate = "" Then
				If $Silent = 0 Then MsgBox(4 + 262144, "SciTE4AutoIt3", "No updates available.")
				Exit
			EndIf
