...
Func _DebugBugReportEnv($err=@error, $ext=@extended)
	...
	Return SetError($err, $ext, "Environment = " & @AutoItVersion & $AutoItX64 & $AutoItAnsi & $Compiled & " under  " & @OSVersion & $OSServicePack & " " & @ProcessorArch)
EndFunc
...
; Example .......; Yes
; ===============================================================================================================================
Func _DebugOut(Const $sOutput, Const $bActivate = False)
	...
	Else
		...
		ControlSend($g_hWndDbg, "", "Edit1", String($sOutput) & @CR, 1) 	; Send raw output, so all characters appear in the stream.
	EndIf
EndFunc   ;==>_DebugOut
...
Func _DebugSetup(Const $sTitle = "Debug Info", Const $bBugReportInfos = false)
	...
	If $bBugReportInfos Then _DebugOut(_DebugBugReportEnv() & @CR)
	Return 1
EndFunc   ;==>_DebugSetup
