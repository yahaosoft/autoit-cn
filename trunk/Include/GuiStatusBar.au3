...
;                  Failure      - 0
; Author ........: Gary Frost (gafrost), Steve Podhajecki <gehossafats at netmdc dot com>
...
Func _GUICtrlStatusBar_Create($hWnd, $vPartEdge = -1, $vPartText = "", $iStyles = -1, $iExStyles = 0x00000000)
	...
	Local $aPartWidth[1], $aPartText[1], $hWndSBar, $x, $iStyle = BitOR($__STATUSBARCONSTANT_WS_CHILD, $__STATUSBARCONSTANT_WS_VISIBLE), $iExStyle, $iLast
	...
	If @OSTYPE = "WIN32_WINDOWS"  Then ; not sure if we want this in here for older systems
		$hWndSBar = DllCall("comctl32.dll", "hwnd", "CreateStatusWindow", "long", $iStyle, "str", "", "hwnd", $hWnd, "int", 0)
		$hWndSBar = HWnd($hWndSBar[0])
	Else
		$hWndSBar = HWnd(_WinAPI_CreateWindowEx($iExStyle, "msctls_statusbar32", "", $iStyle, 0, 0, 0, 0, $hWnd))
	EndIf
...
Func _GUICtrlStatusBar_SetTipText($hWnd, $iPart, $sText)
	If $Debug_SB Then _GUICtrlStatusBar_ValidateClassName($hWnd)
	Local $struct_String, $sBuffer_pointer, $struct_MemMap, $Memory_pointer
	
	$struct_String = DllStructCreate("char TipText[" & StringLen($sText) + 1 & "]")
	$sBuffer_pointer = DllStructGetPtr($struct_String)
	...
	Else
		$Memory_pointer = _MemInit($hWnd, StringLen($sText) + 1, $struct_MemMap)
		_MemWrite($struct_MemMap, $sBuffer_pointer, $Memory_pointer, StringLen($sText) + 1)
...
; Description ...: Used for debugging when creating examples
; Syntax.........: _GUICtrlStatusBar_ValidateClassName($hWnd[, $iLine = @ScriptLineNumber])
; Parameters ....: $hWnd        - Handle to the control
; Return values .: None
; Author ........: Gary Frost (gafrost)
; Modified.......:
...
Func _GUICtrlStatusBar_ValidateClassName($hWnd)
	...
	_WinAPI_ValidateClassName($hWnd, "msctls_statusbar32")
