...
#AutoIt3Wrapper_icon=..\..\Aut2Exe\Icons\windows.ico
...
$GUI_WMI_NamespaceLabel  = GuiCtrlCreateLabel("WMI 命名空间(Namespaces)",  20, 30,150, 20)
$GUI_WMI_Namespace       = GuiCtrlCreateCombo("WMI_Namespaces", 20, 50,280, 80)
$GUI_WMI_ClassLabel      = GuiCtrlCreateLabel("WMI 类('Class')",     320, 30,240, 20)
$GUI_WMI_Classes         = GuiCtrlCreateCombo("WMI_Classes",   320, 50,340, 80)
$GUI_AST_ButtonGroup     = GuiCtrlCreateGroup("",               10, 80,660, 50)
$GUI_AST_Run             = GuiCtrlCreateButton("运行",           20,100, 50, 20)
$GUI_AST_CIMv2           = GuiCtrlCreateButton("CIMv2",         80,100, 50, 20)
$GUI_AST_WMISource       = GuiCtrlCreateButton("WMISource",    140,100, 70, 20)
$GUI_AST_Open            = GuiCtrlCreateButton("打开",         220,100, 60, 20)
$GUI_AST_Save            = GuiCtrlCreateButton("保存",         290,100, 60, 20)
$GUI_AST_Quit            = GuiCtrlCreateButton("退出",         360,100, 60, 20)
$GUI_AST_OptionGroup	 = GuiCtrlCreateGroup("输出",        430, 80,240, 50)
...
$GUI_AST_ScriptCode      = GuiCtrlCreateEdit("载入中,请等待一会儿...",   20,140,640,390)
...
Func LoadWMINamespaces()
   ...
   $strWaitNamespaces="请稍候, 正在载入 WMI 命名空间"
...
Func HandleNamespaceChange()
   ...
   $strWMIWaitMsg = "请稍候, 尝试载入命名空间里面的'类'(class)" & $strSelectedNamespace
...
Func LoadWMIClasses()
	...
	If not @error Then
		...
		If $objClassDictionary.Count Then
			...
			$strClassesCombo="|选择一个 WMI 类('class')" 
...
Func ComposeCode()
	...
	If $strSelectedClass <> "Select a WMI class" Then
		...
		If IsObj($objClass) Then
			...
			$strScriptCode = ""
			$strScriptCode = $strScriptCode & "; 生成于 AutoIt Scriptomatic" & @CRLF & @CRLF
			...
			$strScriptCode = $strScriptCode & '   Msgbox(0,"WMI 输出","No WMI Objects Found for class: " & ' & Chr(34) & $strSelectedClass & Chr(34) & ' )' & @CRLF
...
Func RunScript()
   ...
   ;'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
   $strCmdLine = @AutoItExe & " /AutoIt3ExecuteScript " & $strTmpName
...
Func SaveScript()
   ...
   $strTmpName=FileSaveDialog("保存脚本",@DesktopDir,"AutoIt3 脚本 (*.au3)",16, GUICtrlRead ( $GUI_WMI_Classes ))
...
Func OpenScript()
   ...
   $strTmpName=FileOpenDialog("打开脚本",@DesktopDir,"AutoIt3 脚本 (*.au3)")
