...
GUICtrlSetImage (-1, "objsel.dll",2) ; PTREX
...
Func OpenDatabase($DBPath1)
	...
	If $DBPath1 = "" Then
		$DBPath = FileOpenDialog("New or Existing Database...", "C:\Kevin\AutoItScripts\PartsDatabase", "DB (*.db;db3)|All (*.*)") 
	...
	if $dbpath <> "" Then
		...
		While $GetTableNames.step () = 0
			...
			GUICtrlSetImage (-1, "mmcndmgr.dll",13) ; PTREX
...
$path = FileOpenDialog("Excel File...", "C:\Kevin\AutoItScripts\PartsDatabase", "XLS (*.xls)")
...
Func _Tab_Switch() ; PTREX
	Local $Tabcount = _GUICtrlTabGetItemCount($Tab1)
	Local $State1 = GUICtrlRead ($Tab1)
    If WinActive($Main) Then
	  If GUICtrlRead ($Tab1) = $Tabcount -1 Then	
		$State1 = 0
		_GUICtrlTabSetCurFocus ($Tab1, $State1)
	  Else 
		_GUICtrlTabSetCurFocus ($Tab1, $State1+1)
